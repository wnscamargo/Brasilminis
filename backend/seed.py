...
def _build_products():
    ...
    p.append(_product("Porsche 911 GT3 RS 1:64", 89.90, "diecast-1-64", "miniaturas", "mini-gt", [d[4], d[5]], 24,
                      ["LANÇAMENTO"], featured=True, specs={"Escala": "1:64", "Material": "Zamac", "Rodas": "Borracha real"}))
    p.append(_product("Nissan Skyline GT-R R34 1:64", 74.90, "jdm-legends", "miniaturas", "kaido-house", [d[3], d[2]], 18,
                      ["TREASURE HUNT"], featured=True, specs={"Escala": "1:64", "Série": "Kaido House x Mini GT"}))
    p.append(_product("Ford Mustang Boss 302 1:64", 59.90, "diecast-1-64", "miniaturas", "greenlight", [d[0]], 40,
                      ["NOVO"], desc="Clássico muscle car americano em acabamento premium.", specs={"Escala": "1:64"}))
    p.append(_product("Toyota Supra MK4 JDM 1:64", 99.90, "jdm-legends", "miniaturas", "inno64", [d[1]], 12,
                      ["SUPER TH"], compare=129.90, featured=True, specs={"Escala": "1:64", "Edição": "Limitada"}))
    p.append(_product("Lamborghini Aventador 1:18", 349.90, "diecast-1-18", "miniaturas", "tarmac-works", [m[0], m[2]], 8,
                      ["PREMIUM", "FRETE GRÁTIS"], featured=True, specs={"Escala": "1:18", "Portas": "Abrem"}))
    p.append(_product("McLaren 720S Amarelo 1:18", 379.90, "supercarros", "miniaturas", "tarmac-works", [m[3], m[1]], 6,
                      ["EDIÇÃO LIMITADA"], compare=449.90, specs={"Escala": "1:18"}))
    p.append(_product("BMW M3 E30 Preto 1:64", 69.90, "diecast-1-64", "miniaturas", "tomica", [d[6]], 30,
                      ["NOVO"], specs={"Escala": "1:64"}))
    p.append(_product("Honda Civic Type R 1:64", 64.90, "jdm-legends", "miniaturas", "majorette", [d[2]], 22,
                      ["PROMOÇÃO"], compare=84.90, specs={"Escala": "1:64"}))
    p.append(_product("Chevrolet Camaro SS 1:64", 54.90, "diecast-1-64", "miniaturas", "m2-machines", [d[0], d[3]], 35,
                      [], specs={"Escala": "1:64"}))
    p.append(_product("VW Fusca Racing 1:64", 49.90, "diecast-1-64", "miniaturas", "hot-wheels", [d[1]], 50,
                      ["NOVO"], featured=True, specs={"Escala": "1:64"}))
    p.append(_product("Set Treasure Hunt 2025 (5 peças)", 199.90, "treasure-hunt", "colecionaveis", "hot-wheels", [d[0], d[1]], 10,
                      ["TREASURE HUNT", "FRETE GRÁTIS"], featured=True))
    p.append(_product("Super TH Datsun 240Z", 159.90, "super-th", "colecionaveis", "hot-wheels", [d[3]], 5,
                      ["SUPER TH", "PRÉ-VENDA"], compare=189.90))
    p.append(_product("Diorama Posto Retrô 1:64", 129.90, "dioramas", "colecionaveis", "greenlight", [ac[0]], 14,
                      ["LANÇAMENTO"], desc="Cenário detalhado de posto de gasolina retrô."))
    p.append(_product("Garagem 3 Andares 1:64", 179.90, "garagens", "colecionaveis", "greenlight", [ac[1]], 9,
                      ["NOVO"]))
    # Acessórios
    p.append(_product("Display Acrílico 12 Nichos", 149.90, "displays", "acessorios", "", [ac[0], ac[2]], 20,
                      ["FRETE GRÁTIS"], featured=True, desc="Organize e exponha sua coleção com elegância."))
    p.append(_product("Vitrine LED 1:18", 259.90, "vitrines", "acessorios", "", [ac[1]], 7,
                      ["PREMIUM"], compare=299.90))
    p.append(_product("Case Acrílico Individual (10un)", 79.90, "cases-acrilicos", "acessorios", "", [ac[2]], 60,
                      []))
    p.append(_product("Kit Fita LED USB Colecionador", 89.90, "led", "acessorios", "", [ac[0]], 25,
                      ["NOVO"]))
    p.append(_product("Kit de Limpeza Premium", 44.90, "kits-limpeza", "acessorios", "", [ac[2]], 40,
                      ["PROMOÇÃO"], compare=59.90))
    # Vestuário
    p.append(_product("Boné Brasil Minis Racing", 89.90, "bones", "vestuario", "", [a[0]], 30,
                      ["NOVO"], featured=True))
    p.append(_product("Camiseta JDM Legends", 79.90, "camisetas", "vestuario", "", [a[2]], 45,
                      ["LANÇAMENTO"]))
    p.append(_product("Moletom Premium Carbon", 189.90, "moletons", "vestuario", "", [a[1], a[3]], 20,
                      ["FRETE GRÁTIS"], featured=True))
    p.append(_product("Jaqueta Racing Team", 349.90, "jaquetas", "vestuario", "", [a[3]], 8,
                      ["PREMIUM"], compare=399.90))
    p.append(_product("Mochila Colecionador", 219.90, "mochilas", "vestuario", "", [a[1]], 15,
                      []))
    # Presentes
    p.append(_product("Gift Card R$ 150", 150.00, "gift-card", "presentes", "", [IMG["hero"]], 999,
                      ["NOVO"]))
    p.append(_product("Mystery Box Colecionador", 149.90, "mystery-box", "presentes", "", [d[0], d[1]], 30,
                      ["PROMOÇÃO", "FRETE GRÁTIS"], compare=199.90, featured=True,
                      desc="5 miniaturas surpresa selecionadas pela nossa curadoria."))
    p.append(_product("Combo Iniciante (3 minis + display)", 219.90, "combos", "presentes", "", [ac[0], d[2]], 18,
                      ["PROMOÇÃO"], compare=269.90))
    p.append(_product("Kit Presente JDM", 189.90, "kits", "presentes", "", [d[1], a[2]], 22,
                      ["NOVO"]))
    return p
...
async def seed_data():
    ...
    if await db.brands.count_documents({}) == 0:
        await db.brands.insert_many([
            {"id": str(uuid.uuid4()), "name": b, "slug": slugify(b), "logo": "",
             "description": f"Produtos oficiais {b}."}
            for b in BRANDS
        ])

    if await db.products.count_documents({}) == 0:
        await db.products.insert_many(_build_products())
