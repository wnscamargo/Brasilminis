...
/**
 * Dados de DEMONSTRAÇÃO (local/homologação apenas).
 * Bloqueado em produção para evitar credenciais conhecidas e produtos fictícios.
 * Em produção use: php artisan bm:create-admin
 */
class DemoSeeder extends Seeder
{
    ...
    public function run(): void
    {
        if (app()->environment('production')) {
            $this->command?->warn('DemoSeeder ignorado em produção. Use: php artisan bm:create-admin');
            return;
        }

        $this->callOnce(EssentialSeeder::class);

        // Usuários DEMO (somente fora de produção)
        ...
        $catBySlug = Category::pluck('id', 'slug')->all();
        $brandBySlug = Brand::pluck('id', 'slug')->all();
